...
export type GateReceiptV11 = {
  ...
  go_receipt?:{clauses?:Array<Record<string,unknown>>;used_claim_ids?:string[];receipt_id?:string};
};
export type MotionPoint = {step:number;x:number;y:number;yaw:number};
export type AgentGeometry = {
  collision_width_m:number;
  source:string;
};
...
export type EpisodeBundle = {
  ...
  episode_meta:{replay_id:string;profile:string;seed:number;policy:string;runtime:string;device:string;gpu:string;corridors:Array<{id:string;region:number[];width:number}>;agent_geometry?:{carrier:AgentGeometry;scout:AgentGeometry};recorded_gpu_evidence:boolean;simulation_only:boolean;frozen_candidate_artifact:boolean;live_gpu_dependency:boolean};
