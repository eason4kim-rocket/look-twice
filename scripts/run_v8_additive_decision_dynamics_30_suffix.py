#!/usr/bin/env python3
"""Run the checkpointed 30-body suffix of the two-shard dynamics supplement.

The formal run consumes only the immutable decisions for seeds 102520-102529,
using their original frozen trial-layout indices 20-29.  All 10 independent
worlds (three non-fixed robots per world) are added before one ``scene.build()``
call and are then actuated in fixed seed order.  A
...
fresh single Genesis scene.  Together with the separately completed 60-body
prefix, this can support only a two-scene, cumulative-90/max-co-resident-60
claim; it never supports a claim that 90 robots were co-resident.
"""
...
ENGINEERING_SEEDS = (170862, 170863)
ENGINEERING_LAYOUT_INDICES = (20, 21)
...
DEFAULT_PREFIX_REPORT = (
    ROOT
    / "release"
    / "v8-derived"
    / "decision_dynamics_single_scene_60_102500_102519"
    / "REPORT.json"
)
DEFAULT_PROTOCOL = ROOT / "docs" / "V8_ADDITIVE_DECISION_DYNAMICS_30_SUFFIX_PROTOCOL.md"
DEFAULT_VERIFIER = (
    ROOT / "scripts" / "verify_v8_additive_decision_dynamics_30_suffix.py"
)
...
def extend_prefix_sha256(previous_prefix_sha256: str, checkpoint_sha256: str) -> str:
    return sha256_bytes(
        canonical_json(
            {
...
def new_attempt_identity() -> dict[str, Any]:
    ...
    attempt_id = sha256_bytes(
        f"{utc_now()}:{platform.node()}:{process_id}:".encode("utf-8") + os.urandom(32)
    )
    return {
    ...
    if engineering_smoke:
        return [
            ...
            DecisionCase(
                ...
                corridor_b_blocked=False,
                archived_episode_sha256="1" * 64,
                input_class="engineering_smoke",
            ),
        ]
    cases = load_formal_cases(input_report)[20:30]
    if tuple(case.seed for case in cases) != FORMAL_SEEDS:
        raise RuntimeError("immutable challenge report formal suffix mismatch")
    if not all(case.full_chain_direct for case in cases):
        raise RuntimeError(
            "immutable challenge report suffix is not ten direct decisions"
        )
    return cases
...
def scientific_parameters(args: argparse.Namespace) -> dict[str, Any]:
    ...
    layout_indices = (
        ENGINEERING_LAYOUT_INDICES if args.engineering_smoke else FORMAL_LAYOUT_INDICES
    )
    return {
        ...
        "original_v1_layout_indices": list(layout_indices),
...
def build_source_binding(
    ...
    prefix_report_sha = sha256_file(DEFAULT_PREFIX_REPORT)
    if prefix_report_sha != EXPECTED_PREFIX_REPORT_SHA256:
        raise RuntimeError(
            "completed 60-body prefix report mismatch: "
            f"expected {EXPECTED_PREFIX_REPORT_SHA256}, observed {prefix_report_sha}"
        )
    v1_runner_sha = sha256_file(paths["v1_runner"])
    if v1_runner_sha != EXPECTED_V1_RUNNER_SHA256:
        raise RuntimeError(
            "frozen V1 runner mismatch: "
            f"expected {EXPECTED_V1_RUNNER_SHA256}, observed {v1_runner_sha}"
        )
    git_status = git_output("status", "--porcelain", "--untracked-files=all")
    if not args.engineering_smoke and git_status != "":
        raise RuntimeError(
            f"formal 30-body suffix run requires a clean source tree; observed:\n{git_status}"
        )
    return {
        ...
        "prior_completed_evidence": {
            "path": _relative(DEFAULT_PREFIX_REPORT),
            "sha256": prefix_report_sha,
        ...
        "integrity_rule": (
            ...
            "omitted, or combined across attempts into a full suffix result. "
            "The completed prefix is bound only to define the honest two-shard "
            "claim and is never consumed by the controller."
        ),
    }


def binding_errors(binding: Mapping[str, Any], args: argparse.Namespace) -> list[str]:
    errors: list[str] = []
...
def checkpoint_errors(
    ...
    *,
    ...
    require(
        "trial.original_v1_layout_index",
...
def summarize_trials(
    trials: Sequence[dict[str, Any]], run_class: str
) -> dict[str, Any]:
    expected = {
        "formal": {"trials": 10, "direct": 10, "safe_detour": 0, "floor": 0.15},
        "engineering_smoke": {
            "trials": 2,
            "direct": 2,
            "safe_detour": 0,
    ...
    reduction = (mean_passive - mean_active) / mean_passive if reduction_valid else 0.0
    ...
    return {
        ...
        "maximum_tilt_deg": max(tilt_values, default=0.0) if metrics_finite else 0.0,
...
def _progress_payload(
    *,
    ...
    layout_indices = (
        ENGINEERING_LAYOUT_INDICES
        if run_class == "engineering_smoke"
        else FORMAL_LAYOUT_INDICES
    )
    return {
        ...
        **dict(attempt_identity),
        ...
        "sealed_original_v1_layout_indices": list(
            layout_indices[: len(checkpoint_paths)]
        ),
        ...
        "next_original_v1_layout_index": layout_indices[len(checkpoint_paths)]
        if len(checkpoint_paths) < len(cases)
        else None,
    ...
    *,
        ...
        checkpoint_index.append(
            {
                "trial_index": index,
                "seed": case.seed,
                "original_v1_layout_index": layout_indices[index],
    ...
    return {
        ...
        "boundary": {
            ...
            "single_scene_30_body_suffix_execution": run_class == "formal",
            ...
            "teardown_guard_enabled": True,
            "claim": (
                "Receipt-bound rigid-dynamics replay of the final 10 archived "
                "decisions in one 30-body Genesis scene; partial checkpoints "
                "preserve completed trials but are not a resumable or standalone "
                "full result."
                if run_class == "formal"
        ...
        "combined_claim_boundary": {
            "suffix_passed": run_class == "formal" and summary["all_passed"],
            ...
            "prefix_report": dict(binding["prior_completed_evidence"]),
            "claim": (
        ...
        "source": {
            ...
            "prior_completed_evidence": binding["prior_completed_evidence"],
        },
        "decision_input": {
        ...
        "protocol": {
            **binding["scientific_parameters"],
            ...
            "post_build_entity_pose_writes": 0,
            "route_binding": (
                "formal suffix active routes are the ten archived direct-corridor "
                "decisions; passive always follows the fixed outer detour"
                if run_class == "formal"
                else "engineering active routes exercise one corridor-a and one "
                "corridor-b direct path; passive always follows the outer detour"
            ),
            "checkpoint_policy": (
...
def run(args: argparse.Namespace) -> int:
    ...
    for index, case in enumerate(cases):
        original_v1_layout_index = layout_indices[index]
        layout = trial_layout(case, original_v1_layout_index)
        ...
        records.append(
            {
                "case": case,
                "layout": layout,
                "original_v1_layout_index": original_v1_layout_index,
    ...
    for trial_index, record in enumerate(records):
        ...
        trial = {
            "seed": case.seed,
            "original_v1_layout_index": record["original_v1_layout_index"],
            "decision": {
        ...
        prepublication_errors = checkpoint_errors(
            checkpoint,
            case=case,
            trial_index=trial_index,
            run_class=run_class,
        ...
        if prepublication_errors:
            raise RuntimeError(
                f"unsealed checkpoint {case.seed} failed prepublication: "
                + "; ".join(prepublication_errors)
            )
